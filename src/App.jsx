...
const [interacReference, setInteracReference] = useState('');
    const [formData, setFormData] = useState({
     ...
     const calculateDiscountValue = (code, subtotal) => {
      ...
      switch (discountRule.valueType) {
        case 'percentage':
          return (subtotal * parseFloat(discountRule.value)) / 100;
        case 'fixed_amount':
...
const getDiscountDisplay = () => {
  ...
  const calculateTotals = () => { 
    ...
    const discount = discountCode && validateDiscountCode(discountCode) 
    ? calculateDiscountValue(discountCode, subtotal)
    : 0;
  
    let taxAmount = 0; 
    const taxableAmount = subtotal + shippingFee - discount; // Include shipping fee in tax calculation 
    if (selectedCountry === 'TUN' || selectedCountry === 'TN') { 
